...
void dop_a1(vertex*& point, record* data, int weight);
void LRprint(vertex*);
			...
			if(strcmp(title,"0")!=0){
				...
				if(search == -1){
					...
					for(int i=search_start;i<search;i++){
						cout <<i<<" "<<mas[i]->data->author<<"  "<<mas[i]->data->title<<"  "<<mas[i]->data->publisher<<"  "<<mas[i]->data->year<<"  "<<mas[i]->data->num_of_page<<endl;
						dop_a1(root, mas[i]->data, mass_random[i]);
					}
				}
			}
			_getch();
...
void dop_a1(vertex*& point, record* data, int weight) {
	...
	else if (data->year == point->data->year){
		//cout << "Next ";

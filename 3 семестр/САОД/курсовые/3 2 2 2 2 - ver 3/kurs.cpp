...
int main() {
	...
	while (true) {
		...
		cout << "  4. Поиск в дереве по ключу (Фамилия)" << endl;
		...
		switch (enter) {
			...
			case 52:
				SetConsoleCP(866);
				cout << endl << "ДБД дерево (Фамилия)"<<endl;
				SetConsoleCP(1251);
				TreePrint(root);
				cout<<endl;
				...
				char temp_name[255];
				cin >> temp_name;
				sym_count = strlen(temp_name);
				char name[30];
				strncpy(name, temp_name, sym_count);
				
				//char date[3];
				//cin >> date;
				SetConsoleCP(1251);
				TreeSearch(root, name, sym_count);
				break;
			case 53:
				CodePrint();
				break;
...
void DBD(Base* data, vertex*& p) {
	...
	else if (compare_name(p->data, data) == 0) {
		DBD(data, p->next);
	}
	else if (compare_name(p->data, data) > 0) {
		DBD(data, p->left);
...
{ 
	char bufferA[30];
	char bufferB[30];
	strcpy(bufferA, a->name);
	strcpy(bufferB, b->name);
	//находим позицию первого пробела
	int space_pos_a = strchr(a->name,' ') - a->name;
	int space_pos_b = strchr(b->name,' ') - b->name;
...
void TreeSearch(vertex* p, char *data, int sym_count){
	...
	if(p!=NULL)           
    {
        if (strncmp(p->data->name, data, sym_count) == 0) {
    		printf("|  %-31s|%-7d|%10s|%22s|\n", p->data->name, p->data->sum, p->data->date, p->data->lawyer);
			TreeSearch(p->next, data, sym_count);
		}
		else if (strncmp(p->data->name, data, sym_count) > 0) {

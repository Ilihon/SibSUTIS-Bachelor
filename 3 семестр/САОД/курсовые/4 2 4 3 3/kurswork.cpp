...
int main()
{
	...
	while (true) {
		...
		cout << "\t3. Поиск в отсортированной базе по ключу (Улица)" << endl;
		cout << "\t4. Поиск в дереве (дом)" << endl;
		...
		case 51:
			...
			char X[3];
			SetConsoleCP(866);
			cout << "Введите ключ (три буквы улицы): ";
			cin >> X;
		...
		case 52:
			...
			short int x;
			weight(mas_sbase);
			for(int i = num_h; i<num_t;i++){
				add(root, mas_sbase[i]->data);
			}
			...
			SetConsoleCP(866);
			cout << "Жителей какого дома хотите увидеть?" << endl;
			cin >> x;
			tree_search(root, x);
			break;
		case 53:
...
void DigitalSort(list *&head, bool flag) {
	...
	for (int j = 10; j >= 0; j--) {
		...
		while (head) {
			int d;
			if (flag == 0) {
				if (head->data->number_home == 0)
					d = 0;
				else
					d = head->data->number_home;
			}
			else {
				if (head->data->street[j] == ' ')
					d = 0;
				else
					d = head->data->street[j] + 129;
			}
			p = Q[d].tail;
			if (Q[d].head == NULL)
				Q[d].head = head;
...
void binary_search(char *x, list *l[N], queue *&turn) {
	...
	while (L < R) {
		m = (L + R) / 2;
		sravn = compare(l[m]->data->street, x);
		if (sravn == 1) {
			L = m + 1;
		}
		else {
			if (sravn == -1) {
				...
				if (sravn == 0) {
					h = m;
					t = m;
					while (compare(l[h - 1]->data->street, x) == 0) {
						h--;
						if (h - 1 == 0)
							break;
					}
					while (compare(l[t + 1]->data->street, x) == 0) {
						t++;
						if (t + 1 == 4000)
							break;
					}
...
void add(vertex *&point, record *x) {
	...
	while (*p) {
		if ((*x).number_home < (*p)->fact->data->number_home) {
			p = &((*p)->left);
		}
		else
			if ((*x).number_home > (*p)->fact->data->number_home) {
...
void QuickSort(int *a, list *mas[N],  int left, int right) {
	int current, i, j, t;
	list *tmp;
	current = mas[left + num_h]->data->number_home; 
	i = left; 
	j = right;

	while (i <= j) {
		while (mas[i + num_h]->data->number_home < current) {
			i++;
		}
		while (mas[j + num_h]->data->number_home > current) {
			j--;
		}
		if (i <= j) {
			t = a[i];
...
void tree_search(vertex *&p, short int x) {
	vertex *tmp = p;
	while (tmp != NULL) {
		if (tmp->fact->data->number_home == x) {
			print_base(tmp->fact);
			return;
		}
		else

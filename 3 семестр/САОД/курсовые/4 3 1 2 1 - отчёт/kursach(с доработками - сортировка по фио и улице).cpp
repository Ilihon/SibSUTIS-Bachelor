			...
			if (v == 3) {
				...
				if (search == -1) {
					...
					do {
						...
						if (strncmp(mas[search]->fio, dte, 3) != 0) {
							search++;
							break;
						}
					} while (true);
...
{ 
	...
	else if (strcmp(a.street, b.street) != 0) {
...
void HeapSort(Base*& Com)
{
	...
	while (L > 0) {
		...
		for (;;) {
			...
			if (j > 4000)
				break;
			if ((j < 4000 - 1) && (compare_name(Com[j], Com[j - 1]) >= 0))
				j++;
			if (compare_name(x, Com[j - 1]) >= 0)
				break;
	...
	while (R > 0) {
		...
		for (;;) {
			...
			if (j > R)
				break;
			if ((j < R) && (compare_name(Com[j], Com[j - 1]) >= 0))
				j++;
			if (compare_name(x, Com[j - 1]) > 0)
				break;
			swap(Com[i - 1], Com[j - 1]);
...
int BSearch(Base** A, char dte[3])
{
	int l = 0, r = N, m = 0, fam_pos = 0;
	while (l <= r) {
		m = (l + r) / 2;

		if (strncmp(A[m]->fio, dte, 3) == 0) {
			return m;
		}
		if (strncmp(A[m]->fio, dte, 3) < 0)
			l = m + 1;
...
void addDBD(Base* data, vertex*& point) {
	...
	else if (compare_street(point->data->street, data->street) == 0) {
...
void Tree_Search (vertex* point, char* data){
    ...
    {
        if(compare_street(point->data->street,data) > 0){

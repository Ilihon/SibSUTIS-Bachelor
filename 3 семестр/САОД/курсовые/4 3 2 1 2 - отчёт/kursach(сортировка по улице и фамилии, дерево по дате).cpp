...
void Obhod(vertex *v);
int compare_date(basa *a,basa *b);
...
{ 
	...
	else if (strncmp(a->a, b->a,3) != 0) {
		return strncmp(a->a, b->a,3);
	}
	return 0;
}

int compare_date(basa *a, char *b) 
{ 
	if(strcmp(&a->e[6], &b[6]) != 0){
		return strcmp(&a->e[6], &b[6]);
	}
	else if (a->e[3] - b[3] != 0) {
		return a->e[3] - b[3];
	}
...
void addAVL(spis *p, vertex *&point)
{
	if (point == NULL) {
		...
		if (compare_date(point->pole, p->data->e) > 0) {
			addAVL(p, point->left);
			if (up == true) {
				if (point->bal > 0) {
					point->bal = 0;
		...
		else if (compare_date(point->pole, p->data->e) < 0) { 
...
void searchAVL (vertex *root, char *data) {
	...
	else if (compare_date(root->pole, data) > 0){
    ...
    if (v == 1 or v == 2 or v == 3) {
            ...
            for (int i = 0; i < 4000; i++) {
                ...
                if (temp % 20 == 0) {
                    ...
                    if ((answer != 1) && (answer != 0)) {				
                    	int s;
                     	cout << "Missclick or u really wanna exit? " << "1 fo true, 0 fo no" << endl;
                     	cin >> s;
                     	if (s == 1){						 						 
                        	break;
                    	}
        ...
        if (v == 2) {
            for (int i = 0; i < 4000; i++) {
                ...
                if (kbhit())
                    if (getch() == 13) {
                        cout << "Stopped " << endl << "Moving next? (Double enter for true)" << endl;
...
void search(basa *mas[], int left, int right){
	...
	for (v = 0; v < 4000; v++) {
		while ( left < right ) {
			mid = (left + right) / 2;
			if (strncmp(b, mas[v]->b,3) < 0) left = mid + 1;
				else right = mid;
			}
			if (strncmp(b, mas[v]->b,3) == 0) {			
				Que[find] = mas[v];
				find +=1;
				qu->data = mas[v];
				push_queue(qu, &tail);
	...
	tree = NULL;
 	for(int i = 0; i < find; i++){
 		if (head != NULL){
            cout << head->data->a << "\t" << head->data->b << "\t" << head->data->c << "\t" << head->data->d << "\t" << head->data->e << endl;
			addAVL(head, tree);
			head = head->next;
		}
		else cout << "kkk" << endl;

...
#include "ui_workplace_widget.h"

#include <QMessageBox>
#include <QAbstractItemView>
#include <QSqlQuery>

workplace_widget::workplace_widget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::workplace_widget)
{
    ui->setupUi(this);
...
void workplace_widget::writeTable() {
...
void workplace_widget::on_pushButton_clicked() {//добавить новую запись
    model->sort(0, Qt::AscendingOrder);
    int size = model->rowCount();
    model->insertRow(size);
}
void workplace_widget::on_pushButton_2_clicked() {//сохранить изменения
        model->submitAll();
}
void workplace_widget::on_pushButton_3_clicked() {//отменить изменения
     model->revertAll();
}

void workplace_widget::on_pushButton_5_clicked() {
...
workplace_widget::~workplace_widget()
{
    delete ui;
}

...
InputDialogZaharowa :: InputDialogZaharowa(QWidget* pwgt): QDialog (pwgt){

    m_ptxtFirstName = new QLineEdit;
    m_ptxtLastName = new QLineEdit;

    QLabel* plblFirtsName= new QLabel("&Имя");
    QLabel* plblLastName= new QLabel ("&Фамилия");

    plblFirtsName->setBuddy(m_ptxtFirstName);
    ...
    ptopLayout->addWidget(plblFirtsName,0,0);
...
QString InputDialogZaharowa:: firstName() const{
    return m_ptxtFirstName->text();
}
QString InputDialogZaharowa:: lastName() const{
